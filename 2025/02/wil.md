...
## 목차
- [5일](#5일)
  - [MySQL의 MVCC](#mysql의-mvcc)
- [7일](#7일)
  - [AWS Aurora vs MySQL](#aws-aurora-vs-mysql)
- [18일](#18일)
  - [DB 트랜잭션이 서드파티 API 호출로 인해 길어질 때 발생하는 문제](#db-트랜잭션이-서드파티-api-호출로-인해-길어질-때-발생하는-문제)
- [23일](#23일)
  - [SELECT FOR UPDATE 와 LOCK IN SHARE MODE 차이](#select-for-update-와-lock-in-share-mode-차이)
- [24일](#24일)
  - [자바의 Integer Cache](#자바의-integer-cache)
- [26일](#26일)
  - [Java 가상 쓰레드의 호출 스택(Call Stack)은 힙(Heap)에 저장된다](#java-가상-쓰레드의-호출-스택call-stack은-힙heap에-저장된다)
...
> [!NOTE]
> MVCC는 SELECT FOR UPDATE, LOCK IN SHARE MODE 같은 락을 거는 쿼리에는 적용되지 않는다.  
