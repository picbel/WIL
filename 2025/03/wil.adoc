...
### ✅ 정리  
- MySQL의 Record Lock은 **인덱스를 기반**으로 작동하며, 인덱스가 없거나 부적절하면 불필요한 락 확산이 발생할 수 있다.  
- 적절한 **인덱스 설계가 동시성과 성능에 큰 영향을 준다**.  
- "레코드 락이 인덱스에만 걸린다" → 정확히는 **인덱스를 통해 찾은 테이블 레코드에 락이 걸린다**는 표현이 더 명확하다.
