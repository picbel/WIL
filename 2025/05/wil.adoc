...
**Exactly-Once Semantics(EOS)**는 메시지가 프로듀서에서부터 컨슈머까지 전달 및 처리되는 과정에서 **정확히 한 번만 처리됨**을 보장하는 것을 의미합니다. Kafka는 다음 설정과 처리 방식을 통해 이를 지원합니다.

#### 1. 프로듀서 측 설정 및 처리

* **멱등성 활성화 (enable.idempotence=true)**
    * 프로듀서가 네트워크 오류 등으로 인해 메시지를 재전송할 때, **단일 메시지의 중복 전송을 방지**한다. 프로듀서가 메시지에 시퀀스 번호를 부여하고 브로커가 이를 추적하여 이미 수신한 메시지는 걸러낸다. 이 설정만으로도 at-least-once에서 effectively-once(정확히는 동일 세션 내에서 메시지 유니크 보장) 수준으로 향상됨.
