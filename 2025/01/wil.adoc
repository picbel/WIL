...
https://charlezz.com/?p=44635

스택풀 코루틴 (Stackful Coroutines)
정의
스택풀 코루틴은 각 코루틴이 독립적인 호출 스택을 가지고 있습니다.
따라서 함수 호출이 깊은 상황에서도 실행을 중단하고 나중에 스택 상태를 복원하여 다시 실행할 수 있습니다.

스택리스 코루틴 (Stackless Coroutines)
정의
스택리스 코루틴은 독립적인 호출 스택을 가지지 않습니다. 대신 **상태 머신(State Machine)**과 같은 구조를 사용하여 특정 지점에서만 중단/재개가 가능합니다.
