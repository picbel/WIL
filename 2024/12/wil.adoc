...
- Edge-Triggered 모드 (ET):
이벤트는 상태 변화가 발생했을 때 한 번만 Ready List에 추가됩니다.
데이터를 모두 읽거나 쓸 때까지 작업을 완료해야 하며, 그렇지 않으면 데이터가 남아 있어도 추가 이벤트를 받을 수 없습니다.

- Level-Triggered 모드 (LT):
FD의 이벤트가 "읽기 가능" 상태로 남아 있는 동안, 데이터를 모두 읽지 않으면 다음 epoll_wait 호출에서 같은 FD가 반복적으로 반환될 수 있습니다.
따라서 모든 데이터를 철저히 읽어야 불필요한 반복 처리가 방지됩니다.
