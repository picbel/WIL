...
4. 네트워크 카드 드라이버에서 데이터 수신:
네트워크 카드에서 데이터를 수신하면, NIC(Network Interface Card)가 데이터를 메모리로 DMA(Direct Memory Access)를 통해 전달합니다.
이후 커널의 네트워크 스택이 데이터를 처리하며, FD의 소켓 버퍼에 데이터를 저장합니다.
FD가 읽기 가능 상태가 되었는지 확인하는 과정에서 소켓 상태 변화 이벤트가 발생하면, 해당 FD가 Ready List로 이동됩니다.
