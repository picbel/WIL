...
[이전 달](https://github.com/picbel/WIL/blob/main/2024/04/wil.md)
[다음 달](https://github.com/picbel/WIL/blob/main/2024/06/wil.md)

# 2024년 05월

## 목차
- [3일](#3일)
  - [DAT (동적 메모리 주소 변환)](#dat-동적-메모리-주소-변환)
- [4일](#4일)
  - [메모리 오버레이](#메모리-오버레이)
- [10일](#10일)
  - [DMA 컨트롤러](#dma-컨트롤러)
- [14일](#14일)
  - [Join 동작 방식](#join-동작-방식)
- [17일](#17일)
  - [복합 인덱스 생성시 고려사항](#복합-인덱스-생성시-고려사항)
- [20일](#20일)
  - [NAT](#nat)
- [24일](#24일)
  - [클로저(Closure)](#클로저closure)
- [26일](#26일)
  - [코루틴 - Stackful, Stackless](#코루틴---stackful-stackless)
- [29일](#29일)
  - [클러스터링된 DB 구조에서 외부 Table을 이용한 join이 가능하다](#클러스터링된-db-구조에서-외부-table을-이용한-join이-가능하다)
- [31일](#31일)
  - [MySql / MariaDB 에서 쿼리 임시테이블 사용여부 확인하기](#mysql--mariadb-에서-쿼리-임시테이블-사용여부-확인하기)
...
### 메모리 오버레이
메모리 오버레이(Memory Overlay)는 프로그램의 일부를 메모리에 로드하고 나머지 부분은 필요 할때 로드하는 방식으로 작동하는 기법을 말합니다.  
메모리 용량이 제한된 시스템에서 큰 프로그램을 실행 할 수 있고, 메모리 사용향을 최적화 하 여 효율적인 시스템 운영을 지원 가능하다
...
정렬 병합 조인은 특히 큰 테이블에 적합하며, 인덱스가 없거나 조인 조건이 복잡한 경우에 효 율적입니다. 사전 정렬이 필요하므로 정렬 비용이 추가될 수 있지만, 전체 테이블을 순차적으로 접근하기 때문에 효율적입니다.
...
1. **IP 주소 절약**:
   ...
   - 사설 IP 주소(예: 192.168.x.x, 10.x.x.x)를 사용하는 여러 장치가 NAT 라우터를 통해 하 나의 공인 IP 주소를 공유하여 인터넷에 접속합니다.
...
2. **목적지 IP 주소 변환**:
   - 외부에서 내부 장치로 패킷이 도착할 때, NAT 라우터는 목적지 IP 주소를 공인 IP에서 사 설 IP로 변환합니다.
...
Java의 경우에는 지역 변수 캡쳐시 final이거나 값에 변경이 없는 변수만 사용 가능하다. (자세한 설명은 지난번에 정리하였다 [링크:지역변수캡쳐](https://github.com/picbel/WIL/blob/main/2024/03/wil.md#지역-변수-캡처))  
...
2. **직접적인 컨텍스트 전환:** 스택풀 코루틴은 다른 코루틴으로 전환할 때 전체 호출 스택을 저장하고 복원할 수 있습니다. 이는 함수 호출이 깊게 중첩된 경우에도 코루틴이 적절히 작동 할 수 있게 합니다.
3. **복잡한 제어 흐름:** 복잡한 함수 호출 및 제어 흐름을 쉽게 처리할 수 있습니다. 이는 일반적인 스레드와 유사하게 작동합니다.
4. **성능 오버헤드:** 스택풀 코루틴은 독립된 스택을 유지해야 하므로 메모리 오버헤드가 크 며, 컨텍스트 전환 비용이 상대적으로 높을 수 있습니다.
...
3. **간단한 제어 흐름:** 일반적으로 간단한 제어 흐름을 처리하는 데 적합합니다. 깊게 중첩 된 함수 호출에는 적합하지 않습니다.
