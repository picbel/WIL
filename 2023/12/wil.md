...
[이전 달](https://github.com/picbel/WIL/blob/main/2023/11/wil.md)
[다음 달](https://github.com/picbel/WIL/blob/main/2024/01/wil.md)

# 2023년 12월

## 목차
- [4일](#4일)
  - [하이버네이트 6부터는 DISTINCT를 생략한다.](#하이버네이트-6부터는-distinct를-생략한다)
- [7일](#7일)
  - [스프링의 Bean Scopes](#스프링의-bean-scopes)
- [12일](#12일)
  - [타임아웃 등 재시도 전략 관련 고민](#타임아웃-등-재시도-전략-관련-고민)
- [18일](#18일)
  - [Kafka는 어떻게 빠른걸까?](#kafka-는-어떻게-빠른걸까)
- [20일](#20일)
  - [kotlin에서의 객체 초기화 순서](#kotlin-에서의-객체-초기화-순서)
  - [코틀린 코루틴 내에서 발생한 자식 코루틴의 예외는 부모 코루틴에게 전파된다.](#코틀린-코루틴-내에서-발생한-자식-코루틴의-예외는-부모-코루틴에게-전파된다)
- [23일](#23일)
  - [@PostConstruct와 DeadLock](#postconstruct와-deadlock)
  - [Spring의 graceful shutdown](#spring의-graceful-shutdown)
...
#### Singleton Scope
싱글톤 빈은 하나의 공유된 인스턴스만 관리됩니다. 스프링 IoC 컨테이너가 해당 빈 정의에 정 의된 객체를 딱 하나만 생성합니다.  
...
어쩔수가 없는게 네트워크 혼선이 있을수도있고 잠시 사용량이 몰려서 실제 처리속도가 느려질 수도있다.  
여러가지 이유로 시간지연이 나타날수 밖에없다. 하지만 그렇다고 작업이 끝날때까지 무한히 기다릴수도 없다.  
보통 그래서 타임아웃을 두는것이 일반적인데 그러다보니 만약 타임아웃이 낫다면 후처리는 어 떻게 해야하는가? 에 대한 고민도 많이 하게된다  
1. 재시도를 한다  
...
스프링 공식문서에서 @PostConstruct를 잘못 사용하면 DeadLock(데드락) 위험이 있다고 경고한 다.  
