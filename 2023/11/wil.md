...
- [22일](#22일)
  ...
  - [빈 의존성 해결 프로세스 (Dependency Resolution Process)](#빈-의존성-해결-프로세-dependency-resolution-process)
...
#### 빈 의존성 해결 프로세스 (Dependency Resolution Process)

Spring 컨테이너는 빈을 관리할 때 다음과 같은 엄격한 순서와 원칙에 따라 의존성을 해결합니다.

1. **구성 메타데이터 로드 (Definition Phase)**: `ApplicationContext`가 생성될 때 Java 설정, XML, 어노테이션 등을 읽어 각 빈의 **설계도(`BeanDefinition`)** 를 생성합니다. 이때 빈의 타입, 스코프, 초기화 방법 등은 결정되지만 실제 빈이 생성되지는 않으며, 의존 관계 역시 '정의'된 상태로만 존재합니다.
2. **사전 검증 및 후처리 (Post-Processing)**: `BeanFactoryPostProcessor`를 통해 생성된 `BeanDefinition`들을 수정하거나 보완합니다. (예: 프로퍼티 값 치환 등)
3. **인스턴스 생성 및 의존성 주입 (Instantiation & Dependency Injection Phase)**: 컨테이너 초기화의 마지막 단계에서 `Lazy-init`이 아닌 모든 싱글톤 빈이 생성됩니다.
   - **재귀적 해결**: Spring은 의존성 그래프를 미리 분석하여 완벽한 생성 순서를 정하지 않습니다. 대신, 특정 빈을 생성하는 과정에서 필요한 의존 빈을 **재귀적으로 요청(`getBean`)하여 생성 및 주입**하는 방식을 사용합니다.
   - **문제 감지**: 이 재귀적 처리 과정에서 누락된 의존성이나 순환 참조 문제가 발견됩니다.
   - **순환 참조 해결**: 세터 또는 필드 주입 방식의 순환 참조는 **3단계 캐시(Singleton Objects, Early Singleton Objects, Singleton Factories)**를 통한 조기 참조(Early Reference) 방식으로 해결됩니다. (단, 생성자 주입은 불가능)
4. **초기화 (Initialization)**: 의존성 주입이 완료된 빈에 대해 초기화 로직을 실행하고 마지막으로 프록시가 적용됩니다.
5. **완성된 빈 제공**: 초기화 및 후처리가 완료된 빈은 최종적으로 싱글톤 캐시(singletonObjects)에 저장되며 이후 getBean() 요청 시 재사용됩니다.

> [!NOTE]
> **순환 의존성 해결**: Spring은 싱글톤 빈에 한해 세터 주입 시 '3단계 캐시'를 활용하여 순환 참조 문제를 해결합니다. 하지만 생성자 주입 시에는 인스턴스화 단계에서 의존성이 필요하므로 순환 참조가 발생하면 `BeanCurrentlyInCreationException`이 발생합니다.
